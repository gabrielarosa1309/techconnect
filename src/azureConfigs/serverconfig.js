...
export const CreateImageFile = async (image) => {
    try {
        ...
        //Criando hash do blob
        const blobName = new Date().toISOString() + "." + image.type.split("/")[1];

        //Criando conexao com o cliente do blob
        const blobService = new BlobServiceClient(`https://${Keys.storageAccountName}.blob.core.windows.net?${Keys.sasToken}`);
        ...
        //Definindo o Content-Type correto (de acordo com o tipo de arquivo)
        const options = {
            blobHTTPHeaders: {
        ...
        //Devolvendo a url de criacao do blob
